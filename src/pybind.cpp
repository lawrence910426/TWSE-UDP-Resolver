...
PYBIND11_MODULE(twse_udp_resolver, m) {
    ...
    py::class_<Packet>(m, "Packet")
        ...
        .def_readwrite("reference_price", &Packet::reference_price)
        .def_readwrite("limit_up_price", &Packet::limit_up_price)
        .def_readwrite("limit_down_price", &Packet::limit_down_price)
        .def_property("symbol_count_note", [](const Packet &p) { return py::bytes(p.symbol_count_note, 2); }, set_char_array<2>(&Packet::symbol_count_note))
